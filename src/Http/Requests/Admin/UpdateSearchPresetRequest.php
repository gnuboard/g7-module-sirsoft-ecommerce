 ...
 * 검색 프리셋 수정 요청
 */
class UpdateSearchPresetRequest extends FormRequest
{
    /**
     * 권한 확인
     *
    ...
    public function rules(): array
    {
        ...
        $targetScreen = $this->route('preset')?->target_screen ?? $this->input('target_screen', 'products');
        ...
        $rules = [
            'name' => [
                ...
                'string',
                'max:100',
                // 중복 이름 체크: 동일 사용자, 동일 화면에서 이름 중복 방지 (현재 프리셋 제외)
        ...
        return HookManager::applyFilters('sirsoft-ecommerce.preset.update_validation_rules', $rules, $this);
    }
