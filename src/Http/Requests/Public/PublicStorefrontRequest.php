 ...
 * 쇼핑 첫 화면 통합 조회 요청 (공개)
 *
 * 상품 목록 필터에 더해 진열 묶음(인기/신상품/최근 본)의 개수와 최근 본 상품 ID 를 받습니다.
 * 개별 엔드포인트가 각각 쓰던 규칙을 그대로 합친 것이므로, 같은 값이면 같은 결과가 나옵니다.
 */
class PublicStorefrontRequest extends FormRequest
{
    /**
     * 권한 확인
     *
    ...
    {
        $rules = [
            // 상품 목록 필터 (PublicProductListRequest 와 동일)
            ...
            // 진열 묶음
            'popular_limit' => ['nullable', 'integer', 'min:0', 'max:50'],
            'new_limit' => ['nullable', 'integer', 'min:0', 'max:50'],
            'recent_ids' => ['nullable', 'string', 'max:500'],
        ];

        return HookManager::applyFilters('sirsoft-ecommerce.product.public_storefront_validation_rules', $rules, $this);
    }
    ...
    public function messages(): array
    {
        $messages = [
            ...
            'popular_limit.integer' => __('sirsoft-ecommerce::validation.public_product.limit.integer'),
            'popular_limit.max' => __('sirsoft-ecommerce::validation.public_product.limit.max'),
            'new_limit.integer' => __('sirsoft-ecommerce::validation.public_product.limit.integer'),
            'new_limit.max' => __('sirsoft-ecommerce::validation.public_product.limit.max'),
            'recent_ids.string' => __('sirsoft-ecommerce::validation.public_product.ids.string'),
            'recent_ids.max' => __('sirsoft-ecommerce::validation.public_product.ids.max'),
        ];

        return HookManager::applyFilters('sirsoft-ecommerce.product.public_storefront_validation_messages', $messages, $this);
    }
}
