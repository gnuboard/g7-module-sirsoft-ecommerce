...
class Upgrade_0_3_0 implements UpgradeStepInterface
{
    ...
    private function seedMailTemplates(UpgradeContext $context): void
    {
        ...
        // EcommerceMailTemplate 모델/시더 클래스가 제거되었습니다.
        // 클래스가 존재하는 경우(beta.2 이전 코드베이스)에만 레거시 시드를 수행합니다.
        if (! class_exists(EcommerceMailTemplate::class) || ! class_exists(EcommerceMailTemplateSeeder::class)) {
            $context->logger->info('[v0.3.0] EcommerceMailTemplate 클래스 부재 — 레거시 시드 스킵 (beta.2 이후 정상)');
